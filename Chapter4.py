...
def attack_dragon(player):
    ...
    if player.level >= required_level:
        attack_dragon_input = input("Do you want to proceed with attacking the dragon (yes/no) ")
        if attack_dragon_input == "yes":
            print("You and the team of men attack the dragon")
            time.sleep(2)
            print("some of the men with you perish to the fire from the dragon")
            time.sleep(2)
            print("Yourself, barely makes it alive, with cuts and bruises everywhere,"
        ...
        time.sleep(2)
        repair_input = input("Do you chase the dragon lord? (yes/no)")
        if repair_input == "yes":
            time.sleep(2)
...
def run(player):
    ...
    while True:
        ...
        print("2. Try to think of another plan")
    ...
    chapter4_complete = check_completion()

    if chapter4_complete:
        return True
    else:
        return False
